...
const getData = (key) => new Promise(res => {
    try { const item = localStorage.getItem(key); res(item ? JSON.parse(item) : []); } 
    catch (e) { console.error(e); res([]); }
});
const setData = (key, val) => new Promise(res => {
    try { localStorage.setItem(key, JSON.stringify(val)); res(); } 
    catch (e) { console.error(e); res(); }
});
...
const getHabitLists = async () => {
    let [habits, hidden] = await Promise.all([getData('habits_list'), getData('hidden_habits')]);
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (!Array.isArray(foodList) || foodList.length === 0) await setData('food_list', DEFAULT_FOODS);
...
function setupEventListeners() {
    ...
    el('set-icon-option').onclick = async (e) => {
        ...
        let top = (window.scrollY + e.clientY) + 20;
        let left = e.clientX;
    ...
    el('delete-option').onclick = async () => {
        if (habitToDelete && confirm(`Delete "${habitToDelete}"?`)) {
            let hList = await getData('habits_list');
            if(Array.isArray(hList)) hList = hList.filter(h => h !== habitToDelete);
            
            let hData = await getData('habit_history');
            if(Array.isArray(hData)) hData = hData.filter(h => h.habit_name !== habitToDelete);
            
            let hidden = await getData('hidden_habits');
            if(Array.isArray(hidden)) hidden = hidden.filter(h => h !== habitToDelete);
            
            await setData('habits_list', hList); 
            await setData('habit_history', hData); 
            await setData('hidden_habits', hidden);
            renderHabitTrackers(); updateWeeklyInsights();
        }
        el('custom-context-menu').classList.add('hidden');
...
async function renderHabitTrackers() {
    ...
    visibleHabits.forEach(h => {
        ...
        btn.className = 'habit-nav-item'; 
        btn.dataset.habit = h;
        const emoji = icons[h] ? `<span class="habit-emoji">${icons[h]}</span>` : '';
        
        // ADDED: Menu Button (⋮)
        btn.innerHTML = `
            <div class="habit-click-area" style="display:inline-flex; align-items:center;">
                ${emoji}<span class="habit-name">${h}</span>
            </div>
            <span class="habit-menu-trigger" style="margin-left:8px; opacity:0.6; padding:0 4px; font-weight:bold;">⋮</span>
        `;
        
        // Click to view grid
        btn.querySelector('.habit-click-area').onclick = (e) => {
            e.stopPropagation();
            showHabitGrid(h);
        };
        
        // Click to Open Menu (The reliable way)
        btn.querySelector('.habit-menu-trigger').onclick = (e) => {
            e.stopPropagation();
            openContextMenu(e, h);
        };

        // Right click (Desktop Backup)
        btn.oncontextmenu = (e) => {
            e.preventDefault();
            openContextMenu(e, h);
        };
        
        // Long Press (Mobile Backup)
    ...
    if (hiddenWrap) {
        ...
        if (hidden.length) {
            ...
            hidden.forEach(h => {
                const pill = document.createElement('span'); pill.className = 'hidden-habit-pill'; 
                pill.textContent = icons[h] ? `${icons[h]} ${h}` : h;
                pill.onclick = async () => {
                    let nextHidden = await getData('hidden_habits');
                    if(Array.isArray(nextHidden)) nextHidden = nextHidden.filter(x => x !== h);
...
function openContextMenu(e, habitName) {
    habitToDelete = habitName;
    const menu = el('custom-context-menu');
    // FIX: Calculate position including scroll
    const x = e.clientX || (e.touches && e.touches[0].clientX);
    const y = e.clientY || (e.touches && e.touches[0].clientY);
    
    // Prevent menu going off right screen edge
    const menuLeft = Math.min(x, window.innerWidth - 160);
    const menuTop = y + window.scrollY;

    menu.style.cssText = `top:${menuTop}px;left:${menuLeft}px;`;
...
function attachHabitLongPress(elm, habitName) {
    ...
    const start = (e) => {
        timer = setTimeout(() => {
            openContextMenu(e.touches ? e.touches[0] : e, habitName);
        }, 600);
    };
...
async function renderHabitGraph(name) {
    ...
    // Mobile: Show fewer days
    const isMobile = window.innerWidth < 600;
    const daysToShow = isMobile ? 84 : 365;
    ...
    for (let i = daysToShow; i >= 0; i--) {
        ...
        day.className = `habit-day ${done ? 'level-3' : 'level-0'}`; 
        day.title = s;
        g.appendChild(day);
    }
}
...
// ... Card generation functions (buildMonthlyCardData, etc) are unchanged from previous versions, 
// but ensure they use getData/setData helpers if used ...
// For brevity, those are omitted here but the logic inside script.js above is complete for dashboard function.
// Just keep buildMonthlyCardData and renderMonthlyCardImage from the previous file if you need the share feature.
